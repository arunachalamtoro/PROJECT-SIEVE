...
import type { ReviewResult, TokenUsage, SifthookdevConfig } from '../types.js';
  ...
  if (config.provider === 'anthropic') {
    ...
    const response = await fetch(baseUrl, {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://github.com/sifthookdev',
        'X-Title': 'Sifthookdev',
      },
      body: JSON.stringify({
        model: config.model,
...
export function formatReviewAsMarkdown(review: ReviewResult): string {
  ...
  lines.push('## 🔬 Sifthookdev Code Review');
  lines.push('');
  ...
  lines.push('');
  lines.push('---');
  lines.push('*Generated by [Sifthookdev](https://github.com/sifthookdev) — AI-powered PR analysis with semantic dependency awareness*');
