...
import type { ReviewResult, TokenUsage, SieveConfig } from '../types.js';
  ...
  if (config.provider === 'anthropic') {
    ...
    const response = await fetch(baseUrl, {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://github.com/project-sieve',
        'X-Title': 'Project Sieve',
      },
      body: JSON.stringify({
        model: config.model,
...
export function formatReviewReport(review: ReviewResult): string {
  ...
  lines.push('║' + '  🔬 SIEVE CODE REVIEW'.padEnd(58) + '║');
  lines.push('╚' + '═'.repeat(58) + '╝');
...
export function formatReviewAsMarkdown(review: ReviewResult): string {
  ...
  lines.push('');
  lines.push('---');
  lines.push('*Generated by [Sieve](https://github.com/project-sieve) — AI-powered PR analysis with semantic dependency awareness*');
